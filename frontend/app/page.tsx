...
type Chunk = { id: number; content: string; created_at: string };
type PreviewDoc = { filename: string; chunks: Chunk[]; total_chunks: number };
type Tab = "chat" | "graph";
...
export default function Home() {
  ...
  async function sendMessage() {
    ...
    try {
      ...
      setMessages(prev => [...prev, { role: "assistant", content: lang === "zh" ? "连接失败，请确认后端服务正在运行。" : "Connection failed." }]);
  ...
  async function uploadFile(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      const res = await axios.post("http://localhost:8000/upload", { content: text, filename: file.name });
  ...
  const t = {
    zh: {
      appName: "RAG Knowledge Base", version: "v0.4.0",
      status: "在线", dbStatus: "知识库",
      connected: "已连接", docChunks: (n: number) => `${n} 个文档块`,
      upload: "上传文档", uploadHint: "拖放或点击上传 .txt / .pdf",
      sidebarNote: "文档上传后自动向量化并构建知识图谱。",
      chatTitle: "知识库问答", chatSubtitle: "基于文档的 AI 问答",
      graphSubtitle: "实体关系可视化",
      emptyTitle: "有什么想问的？",
      ...
      filesLabel: (n: number) => `${n} 个文件`,
    },
    en: {
      appName: "RAG Knowledge Base", version: "v0.4.0",
      status: "Online", dbStatus: "Knowledge Base",
      connected: "Connected", docChunks: (n: number) => `${n} chunks`,
      upload: "Upload", uploadHint: "Drop or click to upload .txt / .pdf",
      sidebarNote: "Documents are auto-vectorized and graph-indexed on upload.",
      chatTitle: "Knowledge Base Q&A", chatSubtitle: "AI answers from your documents",
      graphSubtitle: "Entity relationship visualization",
      ...
      <aside className="w-60 bg-[#18181b] flex flex-col shrink-0 border-r border-[#27272a]">
        ...
        <div className="px-4 py-5 border-b border-[#27272a]">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <p className="text-white text-sm font-semibold leading-none">RAG KB</p>
            ...
            {docCount !== null && (
              <div className="flex items-center justify-between">
                ...
                <button onClick={() => setShowDocs(v => !v)}
                  className={`text-xs px-2 py-0.5 rounded-md transition-colors ${showDocs ? "bg-blue-600 text-white" : "text-[#71717a] hover:text-white"}`}>
        ...
        {/* 文档列表 */}
        {showDocs && (
          <div className="px-3 py-3 border-b border-[#27272a] max-h-52 overflow-y-auto">
            {documents.length === 0
              ? <p className="text-[#52525b] text-xs text-center py-3">{t.noDocsYet}</p>
              : <div className="space-y-1.5">
                  {documents.map(doc => (
                    <div key={doc.filename} className="group bg-[#27272a] hover:bg-[#3f3f46] rounded-lg px-3 py-2 transition-colors">
                      <div className="flex items-start gap-2">
                        <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="#71717a" strokeWidth="2" className="shrink-0 mt-0.5">
                          <path d="M14 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V8z"/><polyline points="14 2 14 8 20 8"/>
                        </svg>
                        <div className="flex-1 min-w-0">
                          <p className="text-white text-xs truncate">{doc.filename}</p>
                          <div className="flex items-center justify-between mt-0.5">
                            <span className="text-[#71717a] text-xs">{doc.chunks} {t.chunks}</span>
        ...
        <div className="px-3 py-3 border-b border-[#27272a]">
          <label className="flex flex-col items-center gap-2 cursor-pointer border border-dashed border-[#3f3f46] hover:border-blue-500 rounded-xl p-4 transition-colors group">
            <div className="w-8 h-8 bg-[#27272a] group-hover:bg-blue-950 rounded-lg flex items-center justify-center transition-colors">
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" className="text-[#71717a] group-hover:text-blue-400 transition-colors">
                <path d="M21 15v4a2 2 0 01-2 2H5a2 2 0 01-2-2v-4"/><polyline points="17 8 12 3 7 8"/><line x1="12" y1="3" x2="12" y2="15"/>
              </svg>
            </div>
            <span className="text-[#71717a] text-xs group-hover:text-blue-400 transition-colors text-center leading-relaxed">{t.uploadHint}</span>
          ...
          {uploadStatus.text && (
            <div className={`mt-2 text-xs rounded-lg px-3 py-2 leading-relaxed ${
              ...
              {uploadStatus.type === "loading" && <span className="inline-block w-1.5 h-1.5 bg-blue-400 rounded-full animate-pulse mr-1.5"/>}
              {uploadStatus.text}
            </div>
          )}
        </div>
          ...
          <div className="flex items-center gap-1 bg-gray-100 rounded-lg p-0.5">
            {(["chat", "graph"] as Tab[]).map(tabId => (
              <button key={tabId} onClick={() => setTab(tabId)}
                className={`flex items-center gap-1.5 px-3.5 py-1.5 rounded-md text-sm font-medium transition-all ${tab === tabId ? "bg-white text-gray-900 shadow-sm" : "text-gray-500 hover:text-gray-700"}`}>
                {tabId === "chat"
                  ...
                  : <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><circle cx="5" cy="12" r="2"/><circle cx="19" cy="5" r="2"/><circle cx="19" cy="19" r="2"/><line x1="7" y1="12" x2="17" y2="6"/><line x1="7" y1="12" x2="17" y2="18"/></svg>
                }
            ...
            <div className="flex items-center bg-gray-100 rounded-lg p-0.5">
              {(["zh", "en"] as const).map(l => (
                <button key={l} onClick={() => setLang(l)}
                  className={`px-3 py-1.5 rounded-md text-xs font-medium transition-all ${lang === l ? "bg-white text-gray-900 shadow-sm" : "text-gray-400 hover:text-gray-600"}`}>
                  {l === "zh" ? "中文" : "EN"}
                </button>
              ))}
            </div>
                  ...
                  <div className="max-w-2xl mx-auto px-6 py-8 space-y-8">
                    {messages.map((msg, i) => (
                      <div key={i} className={`flex gap-3 ${msg.role === "user" ? "flex-row-reverse" : "flex-row"}`}>
                        <div className={`w-8 h-8 rounded-full shrink-0 flex items-center justify-center text-xs font-bold ${msg.role === "user" ? "bg-blue-600 text-white" : "bg-gray-900 text-white"}`}>
            ...
            {(preview || previewLoading) && (
              <div className="w-1/2 border-l border-gray-200 bg-white flex flex-col">
                <div className="h-14 px-5 border-b border-gray-100 flex items-center justify-between shrink-0">
                  <div className="flex items-center gap-2 min-w-0">
                    ...
                    <span className="text-gray-700 text-sm font-medium truncate">
                      {previewLoading ? "加载中..." : preview?.filename}
                    </span>
                    {preview && <span className="text-gray-400 text-xs shrink-0">· {preview.total_chunks} {t.chunks}</span>}
                  </div>
                ...
                <div className="flex-1 overflow-y-auto px-5 py-4 space-y-3">
                  {previewLoading
                    ? <div className="flex items-center justify-center h-32"><div className="flex gap-1.5"><span className="w-2 h-2 bg-gray-300 rounded-full animate-bounce"/><span className="w-2 h-2 bg-gray-300 rounded-full animate-bounce [animation-delay:0.15s]"/><span className="w-2 h-2 bg-gray-300 rounded-full animate-bounce [animation-delay:0.3s]"/></div></div>
