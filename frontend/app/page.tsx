...
export default function Home() {
  ...
  const t = {
    zh: {
      ...
      sidebarNote: "上传文档后，系统会自动将其向量化并存入知识库。",
      chatTitle: "知识库问答", chatSubtitle: "基于你上传的文档回答问题",
      graphTitle: "知识图谱", graphSubtitle: "可视化文档中的实体关系",
      ...
      preview: "预览", closePreview: "关闭", chunkLabel: (i: number, t: number) => `第 ${i} 块 / 共 ${t} 块`,
    ...
    en: {
      ...
      sidebarNote: "Documents are automatically vectorized and stored after upload.",
      chatTitle: "Knowledge Base Q&A", chatSubtitle: "Answers based on your uploaded documents",
      graphTitle: "Knowledge Graph", graphSubtitle: "Visualize entity relationships",
      emptyTitle: "Start asking", emptyDesc: "Upload documents on the left, then ask questions.",
      placeholder: "Ask a question...", footer: "Enter to send · Shift+Enter for new line", you: "You",
      viewDocs: "Documents", noDocsYet: "No documents yet", chunks: "chunks", hidePanel: "Hide",
      preview: "Preview", closePreview: "Close", chunkLabel: (i: number, tot: number) => `Chunk ${i} of ${tot}`,
    ...
    <div className="flex h-screen bg-[#f5f5f0] font-sans">
      ...
      <aside className="w-64 bg-[#1c1c1e] flex flex-col py-6 px-4 shrink-0 overflow-y-auto">
        <div className="mb-8">
          ...
          <p className="text-gray-500 text-xs ml-8">v0.4.0</p>
        </div>
          ...
          <div className="flex items-center gap-6">
            ...
            <div className="flex items-center gap-1 bg-gray-100 rounded-lg p-0.5">
              <button onClick={() => setTab("chat")}
                className={`flex items-center gap-1.5 px-4 py-1.5 rounded-md text-sm font-medium transition-all ${tab === "chat" ? "bg-white text-gray-800 shadow-sm" : "text-gray-400 hover:text-gray-600"}`}>
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M21 15a2 2 0 01-2 2H7l-4 4V5a2 2 0 012-2h14a2 2 0 012 2z"/></svg>
                {t.tabChat}
              </button>
              <button onClick={() => setTab("graph")}
                className={`flex items-center gap-1.5 px-4 py-1.5 rounded-md text-sm font-medium transition-all ${tab === "graph" ? "bg-white text-gray-800 shadow-sm" : "text-gray-400 hover:text-gray-600"}`}>
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><circle cx="5" cy="12" r="2"/><circle cx="19" cy="5" r="2"/><circle cx="19" cy="19" r="2"/><line x1="7" y1="12" x2="17" y2="6"/><line x1="7" y1="12" x2="17" y2="18"/></svg>
                {t.tabGraph}
              </button>
            </div>
            <div>
              <p className="text-gray-500 text-xs">{tab === "chat" ? t.chatSubtitle : t.graphSubtitle}</p>
            </div>
          </div>
              ...
              <div className="w-1/2 border-l border-gray-200 bg-white flex flex-col">
                <div className="px-5 py-4 border-b border-gray-100 flex items-center justify-between shrink-0">
                  <div className="flex items-center gap-2 min-w-0">
                    ...
                    <span className="text-gray-700 text-sm font-medium truncate">{previewLoading ? "加载中..." : preview?.filename}</span>
                    {preview && <span className="text-gray-400 text-xs shrink-0">· {preview.total_chunks} {t.chunks}</span>}
                  </div>
                  <button onClick={() => setPreview(null)} className="text-gray-400 hover:text-gray-600 transition-colors ml-3 shrink-0">
                    <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>
                  </button>
                </div>
                <div className="flex-1 overflow-y-auto px-5 py-4 space-y-3">
                  {previewLoading
                    ? <div className="flex items-center justify-center h-32"><div className="flex gap-1"><span className="w-2 h-2 bg-gray-300 rounded-full animate-bounce"/><span className="w-2 h-2 bg-gray-300 rounded-full animate-bounce [animation-delay:0.15s]"/><span className="w-2 h-2 bg-gray-300 rounded-full animate-bounce [animation-delay:0.3s]"/></div></div>
                    : preview?.chunks.map((chunk, i) => (
                        <div key={chunk.id} className="border border-gray-100 rounded-xl p-4 hover:border-gray-200 transition-colors">
                          <div className="flex items-center justify-between mb-2">
                            <span className="text-xs font-medium text-gray-400 bg-gray-50 rounded-md px-2 py-0.5">{t.chunkLabel(i + 1, preview.total_chunks)}</span>
